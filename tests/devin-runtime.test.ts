...
import { parseDevinJson, runDevinModel } from '../src/engines/devin-runtime.js'
...
function mockNative(options: { costTier?: string; actualModel?: string; tools?: unknown[]; answer?: string; abort?: () => void } = {}) {
  return vi.spyOn(processRunner, 'runProcess').mockImplementation(async opts => {
    ...
    writeFileSync(exported, JSON.stringify({ steps: [{ source: 'agent', message: options.answer ?? '{"text":"ROLE_OK"}',
      extra: { generation_model: options.actualModel ?? 'swe-1-7' }, tool_calls: options.tools ?? [] }],
      final_metrics: { total_prompt_tokens: 11, total_completion_tokens: 3, total_cached_tokens: 2 } }))
    return { ...proc, stdout: 'Untrusted welcome text and account details must not become the answer' }
  })
}
...
test('native text roles use the isolated native login and validated export; never HTTP, Codex or stdout', async () => {
  ...
  expect(fetchFn).not.toHaveBeenCalled(); expect(runner).toHaveBeenCalledTimes(2)
  const call = runner.mock.calls[1]![0], isolated = JSON.parse(readFileSync(call.args[1]!, 'utf8'))
...
test.each([{ actualModel: 'paid' }, { tools: [{ function_name: 'exec' }] }, { answer: '' }])('export identity, no-tools and terminal answer are mandatory: %j', async options => {
  mockNative(options)
