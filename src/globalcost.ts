...
export function globalCostReport(cfgPath: string, nowIso: string, extraScopes: ExtraBillingScope[] = []) {
  ...
  const billDb = (label: string, dbFile: string, offset: number, subscriptions: string[]) => {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        if (a?.version !== 1 || !['provider-reported', 'configured-estimate', 'failure-estimate'].includes(a.costSource ?? '')) { fail(label, 'unresolved metered cost provenance'); break }
      }
    } finally { db.close() }
  }
  try {
    ...
    for (const f of readdirSync(dir).filter(n => n.endsWith('.json') && !n.endsWith('.example.json')).sort()) {
      try {
        ...
        billDb(f, dbFile, offset, subscriptions)
  ...
  for (const scope of extraScopes) {
    try {
      for (const dbFile of issueBillingDbs(scope.dir)) billDb(dbFile, realpathSync(dbFile), scope.offset, scope.subscriptions)
...
export function globalBilledToday(cfgPath: string, nowIso: string, extraScopes?: ExtraBillingScope[]): number {
  const report = globalCostReport(cfgPath, nowIso, extraScopes)
