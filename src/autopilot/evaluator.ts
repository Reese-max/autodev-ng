...
export async function evaluate(deps: EvalDeps, goal: Goal, cwd: string): Promise<ProgressSnapshot> {
  ...
  // 無可量測條件：informed LLM 判定——讀 goal.evidenceFiles 內容餵 judge，回 0~10 品質分 +
  // 達成與否（fail-open：讀不到的檔跳過、亂格式或空回應 = 未達成 score 0）。
  ...
  const achieved = /\bACHIEVED\b/i.test(out) && !/NOT-?YET/i.test(out)
  return { achieved, score, detail: out.slice(0, 200) || 'agent 無回應' }
}

// 讀佐證檔：每檔上限 8KB、總上限 24KB，讀不到的檔跳過並註記（fail-open）。回空字串表示無佐證。
