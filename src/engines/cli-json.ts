...
export async function codexJson<T>(cfg: JsonModelOptions, schema: z.ZodType<T>, prompt: string): Promise<T> {
  ...
  const admission = await nativeAdmission('codex', { command: 'codex', model: cfg.model, env, replaceEnv: true, args: ['--ignore-user-config', '--ignore-rules'] })
  const blocked = admissionFailure(admission)
  if (blocked) {
    writeFileSync(join(dir, 'telemetry.json'), JSON.stringify({ at: new Date().toISOString(), phase: 'admission', admission, modelCalls: 0 }))
    throw new Error('CLI model admission failed: ' + blocked.detail)
  }
  ...
  const events = cliEvents(result.stdout) as { type?: string; usage?: { input_tokens?: number; output_tokens?: number; total_tokens?: number }; message?: string }[]
  ...
  writeFileSync(join(dir, 'telemetry.json'), JSON.stringify({ at: new Date().toISOString(), command: 'codex exec', model: cfg.model, effort: cfg.effort, exitCode: result.exitCode, timedOut: result.timedOut, durationMs: result.durationMs,
    admission, usage: events.find(e => e.type === 'turn.completed')?.usage, diagnostics: cliDiagnostic(result, env),
    errors: events.filter(cliError).map(e => redactCli(JSON.stringify(e), env).slice(0, 2000)) }))
  const terminal = events.slice().reverse().find(e => e.type === 'turn.completed' || e.type === 'turn.failed')
  if (result.exitCode !== 0 || result.timedOut || terminal?.type !== 'turn.completed') throw new Error('CLI model failed; no fallback approval: ' + cliDiagnostic(result, env).slice(0, 700))
