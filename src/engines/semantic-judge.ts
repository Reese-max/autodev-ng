...
export async function judgeCommit(opts: LlmOpts, claim: string, diff: string): Promise<{ verdict: JudgeVerdict; detail: string }> {
  if (!opts.url && opts.transport !== 'cli') return { verdict: 'SKIP', detail: 'no judgeUrl configured' }
  ...
  if (result.error) return { verdict: 'SKIP', detail: `judge error: ${result.error}` }
