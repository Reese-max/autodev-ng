...
先建立子問題清單：每項只處理一個可驗證的問題，列出範圍、相依順序、驗證方式與通過標準；仍太大的子問題再細拆。
按相依順序逐項處理，每項都執行檢查並記錄結果，通過後才處理下一項；卡住時針對當前子問題定位，不反覆重做已驗證部分。
改採「最小重現 → 追查所有呼叫端與共用根因 → 選擇不同實作 → 完整回歸」流程。
先用可執行檢查重現問題，再修改程式；說明新方案與原方案有何實質差異，不得只重送原補丁。
所有子問題完成後，整合改動並重新執行原始完整驗收；子問題各自通過仍不能宣稱原任務完成，必須確認整體行為與相互影響。
