...
function timedOut(startMs: number, timeoutMs: number, nowMs: () => number): boolean {
...
export function recentRunStats(dbFile: string, opts: RunStatsOptions = {}): RunStatsResult {
  ...
  const readNowMs = opts.nowMs ?? Date.now
  ...
  try {
    ...
    if (timedOut(started, timeoutMs, readNowMs)) return freshCached(key, nowMs, Number.POSITIVE_INFINITY) ?? reuse('timeout')
