...
import { readState, issueDir } from '../dist/github/state.js'
...
try {
  ...
  assert(values.config && /^[1-9]\d*$/.test(values.issue ?? ''), '--config and --issue are required')
  const cfg = loadGithubConfig(resolve(values.config)), source = JSON.parse(readFileSync(cfg.sourceConfig, 'utf8'))
  ...
  const state = readState(cfg, Number(values.issue))
  assert(state && ['ready', 'published'].includes(state.status), `Live repair is ${state?.status ?? 'missing'}; ${state?.detail ?? 'no worker evidence'}`)
  const manifest = delivery(cfg, state), receipts = join(issueDir(cfg, state.issue.number), 'codex-home', 'receipts')
