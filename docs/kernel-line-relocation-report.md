...
`src/*.ts` 頂層由 **2700 行**降至 **2351 行**，實際騰回 **349 行**。因此符合頂層最多
2450 行，且至少騰回 250 行的目標。

| 量測點 | 來源 | 頂層行數 | 與搬移前差異 |
| --- | --- | ---: | ---: |
| 搬移前 | `942554a00fdbed0b6666ff6f4a9bac140809fdce` | 2700 | 0 |
| 搬移後 | 目前工作樹 `src/*.ts` | 2351 | -349 |
...
`src/scheduler.ts` 再為簽名熔斷告警接線與 commit 自證行增加 7 行，目前淨騰回 349 行。
...
2700 → 2351、349 行騰回、≤2450 與 ≥250。第二個測試持續守住既有 kernel 預算與薄殼邊界。
