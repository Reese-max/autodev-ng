...
`src/*.ts` 頂層由 **2700 行**降至 **2244 行**，實際騰回 **456 行**。因此確認頂層嚴格低於
2250 行，且至少騰回 250 行。

| 量測點 | 來源 | 頂層行數 | 與搬移前差異 |
| --- | --- | ---: | ---: |
| 搬移前 | `942554a00fdbed0b6666ff6f4a9bac140809fdce` | 2700 | 0 |
| 搬移後 | 目前工作樹 `src/*.ts` | 2244 | -456 |
...
2700 → 2244、456 行騰回、<2250 與 ≥250，並逐檔比對上表及搬移目的地。第二個測試持續守住 ≤2250 的
